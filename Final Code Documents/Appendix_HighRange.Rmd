...
```{r}
pcor(dependent, method = "pearson") #AT &TIT & CDP problematic 
```
...
```{r}
#Outlier inspection & removal 
critval = qt(0.05/(2*nobs(mod_gt_high_log_try)), df=df.residual(mod_gt_high_log_try)-1, lower=FALSE)
out_ind = which(abs(rstudent(mod_gt_high_log_try)) > critval)
out_ind
```
...
```{r}
final <- subset(gt_adj, select = -c(AT, TIT, CDP))
corrgram(final)
round(cor(final),2)
vif(mod_gt_high_log_1)
```
...
Predicted effect in CO when changing one predictor while holding others constant. (For the final model without NOX).
