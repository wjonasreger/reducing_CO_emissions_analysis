---
title: "STAT 443 Appendix for Mid-Range Analysis"
...
pred.mb = function(x, mbc, chng, i, name){
  ...
  plot(ms, c, main=paste(name, "SE = ", round(sd(chng), 2), "vs. CO predicted change"),
       xlab=paste0(name, ": Min 2se = ", round(minval, 1), ", Min 1se = ", round(minval1se, 1)), 
       ylab = paste0("CO diff (", round(max(c)-min(c), 2), ")"), 
       sub = paste0("CO: Min 1se = ", round(min(c), 2), ", Min 1se = ", round(min(c[26:75]), 2)),
       type='l', col="#13294b")
...
par(mfrow=c(2,2))
plot(mid1)
run_all_diagnostic(mid1)
par(mfrow=c(2,2))
plot(midbest1)
run_all_diagnostic(midbest1)
```
...
par(mfrow=c(2,2))
plot(full1)
run_all_diagnostic(full1)
par(mfrow=c(2,2))
plot(j_mod)
run_all_diagnostic(j_mod)
```
...
```{r}
NOx.lm = lm(NOX ~ ., data = gt_mid_all)
summary(NOx.lm)
summary(update(NOx.lm, data=gt_adj_all))

summary(update(NOx.lm, .~.-CO))
summary(update(NOx.lm, .~.-CO, data = gt_adj_all))
```
